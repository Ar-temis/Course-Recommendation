...
const generateBotResponse = async (incomingMessageDiv) => {
    ...
    while (true) {
        ...
        // Convert entire buffer to HTML using marked.js
        const rendered = marked.parse(buffer);

        // Replace the message with real HTML
...
const HandleOutgoingMessage = (e) => {
    ...
    // User bubble
    const userBubbleHTML = `<div class="message-text">${userData.message}</div>`;
    const outgoingDiv = createMessageElement(userBubbleHTML, "user-message");
    chatBody.appendChild(outgoingDiv);

    chatBody.scrollTop = chatBody.scrollHeight;

    // Bot bubble with thinking animation
    setTimeout(() => {
        const botBubbleHTML = `
        <svg class="bot-avatar" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 122.88 119.35">
        <title>chatbot</title>
        ...
        const incomingDiv = createMessageElement(botBubbleHTML, "bot-message", "thinking");
